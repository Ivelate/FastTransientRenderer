...
void main()
{
	...
	accumRadiance(screenPos,distLightWorld,colorsete);
	...
	for(float w=0.5;w<lightRes_x;w++)
	{
		for(float h=0.5;h<lightRes_y;h++)
		{
			...
			accumRadiance(screenPos,distLightWorld+dist,(colorseteLight*max(0,dot(-vecLightWorld,normalLight))*max(0,dot(vecLightWorld,normalCam))/(dist*dist))*colorseteBase*pxAttenuation);		// if no pxAttenuation, attenuate using /(lightRes_x*lightRes_y));
		}
	}
}