...
public class Camera
{
	...
	{
		this.pos=new Vector3f(MatrixHelper.mulVector(-1,pos,null));
	}
	...
	public void lookTo(Vector3f to)
	{
		Vector3f sub=Vector3f.sub(MatrixHelper.mulVector(-1, to, null), pos, null);
		this.yaw=(float)Math.atan2(-sub.x, sub.z);
		this.pitch=(float)Math.atan2(sub.y, Math.sqrt(sub.x*sub.x+sub.z*sub.z));
	}
